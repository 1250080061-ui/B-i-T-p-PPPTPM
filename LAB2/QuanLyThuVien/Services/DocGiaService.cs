...
namespace QuanLyThuVien.Services
{
    public class DocGiaService
    {
        ...
        {
            return Db.Query(@"SELECT dq.MaDocGia, dq.Ho, dq.Ten, dq.NgaySinh, dq.Phai, dq.SoDienThoai,
                                     dq.DiaChi, dq.Email, dq.Anh3x4,
                                     t.MaThe, t.NgayCap, t.HanSuDung, t.DaDongLePhi, t.TrangThai
                              FROM DocGia dq
                              OUTER APPLY (
                                  SELECT TOP 1 * FROM TheDocGia x
                                  WHERE x.MaDocGia = dq.MaDocGia 
                                  ORDER BY x.TrangThai DESC, x.HanSuDung DESC
                              ) t
                              ORDER BY dq.MaDocGia");
        }

        public DataTable LayComboDocGia()
        {
            return Db.Query("SELECT MaDocGia, (Ho + ' ' + Ten) AS HoTen FROM DocGia ORDER BY Ho, Ten");
        }
        ...
        public KetQuaXuLy Luu(DocGia dg, bool capNhat)
        {
            ...
            try
            {
                ...
                return n > 0
                    ? KetQuaXuLy.Ok(capNhat ? "Cập nhật độc giả thành công." : "Thêm độc giả thành công.")
            ...
            using (SqlTransaction tx = cn.BeginTransaction())
            {
                try
                {
                    ...
                    using (SqlCommand off = new SqlCommand(
                        "UPDATE TheDocGia SET TrangThai=0 WHERE MaDocGia=@Ma AND TrangThai=1", cn, tx))
                    {
        ...
        public KetQuaXuLy GiaHanThe(string maDocGia, DateTime hanMoi, bool daDongLePhi)
        {
            ...
            using (SqlCommand cmd = new SqlCommand(
                @"SELECT TOP 1 MaThe, NgayCap, HanSuDung FROM TheDocGia 
                  WHERE MaDocGia=@Ma AND TrangThai=1 ORDER BY HanSuDung DESC", cn))
            {
                ...
                using (SqlDataReader rd = cmd.ExecuteReader())
                {
                    ...
                    {
                        return KetQuaXuLy.Loi("Hạn sử dụng mới phải sau hạn sử dụng hiện tại.");
                    }
                    rd.Close();

                    using (SqlCommand up = new SqlCommand(
                        "UPDATE TheDocGia SET HanSuDung=@Han, DaDongLePhi=@LePhi WHERE MaThe=@MaThe", cn))
                    {
