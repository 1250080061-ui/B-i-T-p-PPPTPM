    ...
    public class SachService
    {
        public DataTable LayDanhSach(string tuKhoa)
        {
            string sql = @"SELECT s.MaDauSach, s.TenSach, s.NamXuatBan, s.SoLuongHienCo,
                          ...
                          FROM DauSach s
                          LEFT JOIN TheLoai tl ON tl.MaTheLoai = s.MaTheLoai
                          LEFT JOIN NhaXuatBan nxb ON nxb.MaNhaXuatBan = s.MaNhaXuatBan
        ...
        public KetQuaXuLy Xoa(string ma)
        {
            ...
            {
                if (ex.Number == 547)
                    return KetQuaXuLy.Loi("Không thể xóa đầu sách đã phát sinh giao dịch mượn/trả.");
                return KetQuaXuLy.Loi("Lỗi cơ sở dữ liệu: " + ex.Message);
            }
        }
    }
